    ...
    public function toArray($request)
    {
        if (explode('/', $request->getPathInfo())[2] === 'events' && count(explode('/', $request->getPathInfo())) === 4) {
            ...
            return [
                ...
                'level' => $this->name,
                'liveReports' => collect($this->live_reports->load([ 'article_author', 'liveReportPlayers.player', 'liveReportPlayers.player.country' ])->where('poker_event_id', $currentId))->sortByDesc('date_added')->all(),
            ];
        } else {
            return [
